...
v1.1 では、既存の LoDD 原則を広げすぎず、境界 enforcement、結果分類、完了メタデータ、New Chat 継続の4点だけを Framework Reference に昇格する。
これらは特定 Runner の内部実装ではなく、LoDD が「AIを信頼しないことでAIを最大限活用する」ための運用上の最小契約である。

1. **実行境界モデル**
   Prompt-only / repo-root 実行と materialized workspace 実行を区別する。前者は軽量だが LD-001 Read Boundary が指示駆動になる。後者は遅いが、AIに見えるファイル集合そのものを Context Boundary に近づけるため、Read Boundary を構造的に強められる。
2. **Lock-down 結果分類**
   通常の失敗、境界突破、手動検証待ちを同じ「失敗」に畳み込まない。結果状態は `green`, `red`, `breach`, `needs-human-validation` を基準語彙とし、manual / hybrid Done は人間の確認残りを明示する。
3. **完了メタデータの正規化**
   Retrospective と Debt Markers のフィールドを固定し、返済対象の抽出を容易にする。Debt Marker は `unreviewed_functions`, `boundary_issues`, `knowledge_gaps` を canonical schema とする。
4. **New Chat handoff**
   失敗した会話全体を引き継がず、タスク、Context Boundary、Functional Contract、Strict Constraints、最新 delta、残 Debt だけを小さな handoff packet として渡す。

この追加は materialized workspace を全 LoDD 利用に必須化しない。小さく低リスクなタスクでは prompt-only / repo-root 実行で十分な場合がある。v1.1 の目的は、境界を強める選択肢と、結果・継続・負債の表現を揃えることである。
...
LoDDは、LLMによる実装生成を制御するためのフレームワークである。
以下の問題に対処する。

- AIがスコープ外のコードを参照・変更することによる意図しない破壊
- タスク間のコンテキスト汚染による実装精度の劣化
- AI生成コードに潜む負債の不可視な蓄積
- タスク間でコーディングスタイルが分裂する不統一

### LoDDが解決しない問題

LoDDは「定義された仕様に基づいてAIをどう動かすか」を扱う。
以下の問題はこのフレームワークのスコープ外である。

- 仕様の設計: architecture.md, interfaces/, knowledge/ に何を書くべきか
- 仕様の整合性: 定義群が相互に矛盾していないかの検証
- タスク分割の妥当性: 粒度や依存関係の切り方が適切かどうかの判断

これらは人間の設計能力に依存し、LoDDとは別の方法論で対処する必要がある。

### 前提条件

LoDDが正しく機能するためには、以下が満たされていることを前提とする。

- 入力される定義群（architecture.md, interfaces/, knowledge/）の記述内容に内部矛盾がないこと
- タスクが適切な粒度で分割されていること

定義群に矛盾がある場合、LoDDはその矛盾を忠実に実装に反映する。
フレームワークの制御が正常に機能していても、出力の正しさは入力の正しさに依存する。
...
タスク完了時に結果状態と軽量な運用メトリクスを記録する。LoDDが機能しているかを判断するための完了メタデータ。

- **result_state:** `green`, `red`, `breach`, `needs-human-validation` のいずれか。最終結果の分類。manual / hybrid Done で人間の確認が残る場合は `needs-human-validation` を使う。
...
- **manual_follow_up:** manual / hybrid Done で残る人間の確認事項。残りがなければ `None`。

#### Debt Markers

タスク完了時に負債を可視化する。返済スプリントで何をすべきかがここから導出される。各値は `None` または短い plain-text description とする。`None` は「未確認」ではなく、そのカテゴリに active debt がないことを意味する。

- **unreviewed_functions:** 意図を1行で説明できない関数や、人間がまだ確認していない関数。
- **boundary_issues:** Context Boundary の曖昧さ、Read / Write 漏れの疑い、または境界外に触れた可能性。
- **knowledge_gaps:** DCC API、library、環境依存など、人間の検証後に `knowledge/` へ転記すべき知識の不足。
...
### 実行境界レベル（Prompt-only / Materialized）

Context Boundary は task file の Read / Write 定義だけでは完結しない。AI をどの作業空間で実行するかによって、LD-001 の強さが変わる。

| レベル | モデル | 特徴 | 選択基準 |
|---|---|---|---|
| Prompt-only / repo-root execution | AI は repository root など広い作業空間で動き、Read / Write は prompt と事後検証で制御する | 速く単純。Write Boundary は差分検証で強められるが、LD-001 Read Boundary は指示駆動になりやすい | 小さく低リスクなタスク、対象ファイルが明確な修正 |
| Materialized workspace execution | AI に見せる作業空間を task file と Context Boundary に必要なファイルだけで構成する | 準備コストは増えるが、境界外ファイルが存在しないため LD-001 を構造的に強められる | LD-001 が重要なタスク、大規模 repository、偶発的な横読みのコストが高い修正 |

Materialized workspace に入れてよいものは、原則として次に限定する。

- task file
- 適用される `AGENTS.md`
- Phase 構造を使う場合の phase context files
- Context Boundary の `Read` entries
- 既存の Context Boundary `Write` entries
- まだ存在しない Write entries を示す manifest（親 directory の作成は許容するが、内容を勝手に作らない）

Materialization は「念のため repository を広めにコピーする」仕組みではない。境界外ファイルをコピーしてはならない。package manager や dependency resolution を materialization の裏側で実行してはならない。agent workspace で境界外に作られたファイルや変更は、validation workspace に適用してはならない。

Validation workspace は full repository のまま残す。AI の許可された Write だけを適用した後、LD-002, LD-003, LD-004, TBD(LD-) marker check, Done verification を validation workspace で実行する。Materialized workspace は低レベル syscall audit ではなく、最初の実用モデルでは「見えるファイル集合を狭める構造的予防」として扱う。

---

### TBDマーカーの記入規則

AIは違反を検知した時点で、該当コード箇所に以下の形式でマーカーを記入し、作業を停止する。

    # TBD(LD-XXX): 違反内容の1行要約

記入先は違反が発生したソースファイル内の該当行。
人間は grep -r "TBD(LD-" src/ で全違反箇所を一覧できる。

---

### TBDマーカーの解決フロー

人間が TBD マーカーを検知したら、原因を判定する:

a) Context Boundary の漏れ（必要なファイルがRead/Writeに含まれていなかった）
   → task-xxx.md の Context Boundary を修正
   → Context Boundary チェックを再実施
   → TBDマーカーを削除し、New Chat で再投入

b) AIの逸脱（Boundary は正しいが、AIが無視した）
   → task-xxx.md の Strict Constraints に制約を追記
   → TBDマーカーを削除し、Iteration Control を更新、New Chat で再投入

c) モジュール分割の問題（境界自体が不適切）
   → タスクの再分解を検討
   → 必要であれば interfaces/ の更新を先行
...
- Retrospective の result_state, iterations, boundary_breaches, knowledge_added, manual_follow_up を記入する
- Debt Markers を記入する
- iterations/ に knowledge/ に転記すべき内容がないか確認する
  → あれば knowledge/ に転記（人間レビュー済みのもののみ）
  → 設計判断があれば decision_log.md / architecture.md に反映
- iterations/task-xxx/ を削除（パージ）
- AI生成コードの各関数の意図を1行で説明できるか確認する
  → 説明できない関数があれば Debt Markers の unreviewed_functions に追記
...
New Chat 再投入では、前の会話全体を貼り直さない。引き継ぐのは、安定した task context と最新の delta instruction だけである。これは iterations/ の隔離と Delta Recovery を守るための転送 packet であり、task file の代替ではない。

含める内容は次に限定する。

- task ID と title
- 現在の status と current_iteration
- Context Boundary `Read` entries と、それぞれを含める理由
- Context Boundary `Write` entries
- Functional Contract と Done condition
- Strict Constraints
- 最新 failure summary（ある場合）
- current_delta_instruction
- active Debt Markers（ある場合）

明示的に除外するもの:

- 前回 chat 全体
- Context Boundary 外の file context
- 長い stdout / stderr の全文（必要な場合も bounded tail のみ）
- task constraint または knowledge entry に変換されていない推測
- 失敗した実装の長い reasoning log

```markdown
# New Chat Handoff: <task-id> <title>

## Read First
- AGENTS.md
- <task file>
- <Context Boundary Read entry> - <why>
...
この packet は短いほどよい。新しい chat はこの packet と task file から再開し、古い会話の文脈汚染を継承しない。
